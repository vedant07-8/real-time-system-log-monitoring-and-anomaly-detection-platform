...
export default function ToastContainer({ toast, onClose }) {
  ...
  return (
    <div className="fixed top-16 right-4 z-50 max-w-sm w-full pointer-events-auto">
      <div
        ref={containerRef}
        className="bg-[#0d0d0d] border-l-4 border-l-red-500 border border-[#262626] rounded-lg p-3.5 shadow-2xl flex items-start justify-between gap-3"
      >
        <div className="flex items-start gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-200 mt-1 leading-snug">{toast.message}</p>
            {toast.source_ip && (
              <span className="text-[10px] font-mono text-zinc-500 mt-1 block">Origin IP: {toast.source_ip}</span>
            )}
          </div>
        </div>
        <button
          onClick={onClose}
          className="text-zinc-500 hover:text-zinc-300 p-1 rounded hover:bg-zinc-800 shrink-0"
        >
